...
Check these URLs before and during your presentation:

```
# Is the backend alive?
https://your-app.onrender.com/health

# How many Groq API calls today?
https://your-app.onrender.com/quota-status
```

If `/quota-status` returns `"status": "critical"`, switch to demo mode for the rest of the presentation.
...
|---|---|
| **Dev A** — Flutter | UI/UX, camera, voice (TTS/STT), animations, all screens |
| **Dev B** — AI/FastAPI | Groq integration, personalization logic, error handling, deployment |
| **Dev C** — Data/Backend | Supabase schema, Open Food Facts, community stats, notifications |

---

## Known Limitations

- **OCR accuracy** depends on lighting and label print quality. The confirm screen allows manual editing before analysis.
- **Alternatives** rely on Open Food Facts database coverage, which is stronger for international products. Indian brands may have limited coverage.
- **Translation** supports English and Hindi only. Other Indian languages require Bhashini integration (on roadmap).
- **Render free tier** sleeps after 15 minutes of inactivity. First request after sleep takes ~60 seconds. UptimeRobot prevents this during active use.
- **Groq free tier** allows ~500 requests/day. Sufficient for demos and light usage. Production would require a paid plan.

---

## Roadmap

- [ ] iOS support (Flutter app is ready — needs Apple Developer account)
- [ ] Cosmetic product database (Open Beauty Facts integration)
